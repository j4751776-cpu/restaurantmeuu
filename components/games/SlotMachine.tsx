...
const SlotMachine: React.FC<{ theme: Theme }> = ({ theme }) => {
  ...
  const startSpin = (event: React.MouseEvent<HTMLElement>) => {
    ripple(event);
    if (spinning || balance < 5) return;
    ...
    const run = () => {
      ...
      if (steps > 0) {
        ...
        if (wonAmount > 0) sounds.playWin(); else sounds.playLoss();
      }
    };
    run();
  };
  ...
  return (
    <div className="flex flex-col items-center gap-6 max-w-4xl mx-auto p-4 animate-in fade-in duration-500 pb-24">
      <div className={`w-full flex justify-between items-center p-4 rounded-3xl border shadow-2xl ${panelBg}`}>
        ...
        <div className="flex items-center gap-3 bg-yellow-500 text-black px-6 py-2.5 rounded-2xl font-black shadow-lg">
      ...
      <div className="w-full flex flex-col lg:flex-row gap-8 items-start justify-center">
        {/* Roleta Visual */}
        <div className="flex-1 grid grid-cols-3 sm:grid-cols-4 gap-3 p-6 bg-black rounded-[3rem] border-[10px] border-zinc-900 shadow-2xl relative">
          ...
          {SYMBOL_DATA.map((s, i) => (
            <div key={i} className={`relative flex flex-col items-center justify-center rounded-2xl aspect-square border-4 transition-all duration-150 ${activeIndex === i ? 'bg-orange-600 border-white scale-110 shadow-[0_0_40px_rgba(234,88,12,0.8)] z-10' : 'bg-zinc-900 border-white/5 opacity-30 grayscale-[0.5]'}`}>
              <span className="text-3xl sm:text-5xl">{s.char}</span>
              <span className={`text-[8px] font-black mt-1 ${activeIndex === i ? 'text-white' : 'text-zinc-500'}`}>{s.value > 0 ? `$${s.value}` : 'ZERO'}</span>
            </div>
          ))}
        </div>
        ...
        <div className={`w-full lg:w-80 p-8 rounded-[2.5rem] border shadow-2xl flex flex-col items-center text-center gap-6 ${panelBg}`}>
          <div className="space-y-1">
            ...
            <p className="text-[10px] font-bold opacity-40 uppercase tracking-widest">Giro: $5</p>
          </div>
          ...
          <div className="w-full aspect-square max-w-[180px] flex flex-col items-center justify-center border-4 border-dashed border-zinc-500/20 rounded-full p-6 bg-zinc-500/5">
             {spinning && <div className="animate-spin text-orange-600"><RotateCcw size={64} /></div>}
             {!spinning && lastWin !== null && (
               <div className="animate-in zoom-in duration-300 flex flex-col items-center">
                  {lastWin > 0 ? (
                    <>
                      <Trophy size={64} className="text-yellow-500 mb-2 animate-bounce" />
                      <p className="text-emerald-500 font-black text-4xl italic">+${lastWin}</p>
                    </>
                  ) : (
                    <>
          ...
          <button 
            ...
            className="w-full py-6 bg-gradient-to-br from-orange-500 to-red-600 text-white rounded-[2rem] font-black text-2xl italic hover:scale-105 active:scale-95 shadow-2xl disabled:opacity-30 transition-all"
          >
      ...
      <div className={`w-full max-w-2xl p-6 rounded-3xl border ${panelBg} grid grid-cols-2 sm:grid-cols-3 gap-3 opacity-60 text-[10px]`}>
         {SYMBOL_DATA.sort((a,b)=>b.value-a.value).map((s, idx) => (
           <div key={idx} className="flex items-center gap-2">
              <span className="text-lg">{s.char}</span>
              <span className={`font-black ${s.value === 0 ? 'text-red-500' : 'text-emerald-500'}`}>{s.value > 0 ? `$${s.value}` : 'NADA'}</span>
           </div>
         ))}
      </div>
    </div>
